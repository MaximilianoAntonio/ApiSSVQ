...
class DashboardStatsViewSafe(APIView):
    """
    Vista de estadísticas del dashboard - versión segura para Railway
    """
    permission_classes = [IsAuthenticated]
    
    def get(self, request):
        """
        Versión simplificada y segura del dashboard que no debería fallar
        """
        try:
            # Estadísticas básicas con manejo de errores individual
            stats = {}
            
            # 1. Estadísticas generales (con try/catch individual)
            try:
                stats['general'] = {
                ...
                stats['general'] = {'error': f'Error en estadísticas generales: {str(e)}'}
            
            # 2. Vehículos por estado
            try:
                vehiculos_por_estado = {}
                for estado in ['disponible', 'en_uso', 'mantenimiento', 'reservado']:
                    vehiculos_por_estado[estado] = Vehiculo.objects.filter(estado=estado).count()
                stats['vehiculos'] = {'por_estado': vehiculos_por_estado}
            except Exception as e:
                stats['vehiculos'] = {'error': f'Error en estadísticas de vehículos: {str(e)}'}
            
            # 3. Conductores por disponibilidad
            try:
                conductores_por_disponibilidad = {}
                for estado in ['disponible', 'en_ruta', 'dia_libre', 'no_disponible']:
                    conductores_por_disponibilidad[estado] = Conductor.objects.filter(estado_disponibilidad=estado).count()
                stats['conductores'] = {'por_disponibilidad': conductores_por_disponibilidad}
            except Exception as e:
                stats['conductores'] = {'error': f'Error en estadísticas de conductores: {str(e)}'}
            
            # 4. Asignaciones básicas
            try:
                asignaciones_por_estado = {}
                for estado in ['pendiente_auto', 'programada', 'activa', 'completada', 'cancelada']:
                    asignaciones_por_estado[estado] = Asignacion.objects.filter(estado=estado).count()
                stats['asignaciones'] = {'por_estado': asignaciones_por_estado}
            except Exception as e:
                stats['asignaciones'] = {'error': f'Error en estadísticas de asignaciones: {str(e)}'}
            
            # 5. Metadatos
            stats['metadatos'] = {
                'timestamp': timezone.now().isoformat(),
                'version': 'safe',
                'errors': [key for key, value in stats.items() if isinstance(value, dict) and 'error' in value]
            }
            
            return Response(stats, status=status.HTTP_200_OK)
            
        except Exception as e:
            # Error global - esto nos dirá exactamente qué está fallando
            import traceback
            error_details = {
                'error': 'Error crítico en DashboardStatsViewSafe',
                'message': str(e),
